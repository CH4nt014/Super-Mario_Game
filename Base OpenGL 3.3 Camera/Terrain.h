...
public:
	...
	glm::vec3 updateCameraPositionOnMap(glm::vec3 posCamera, glm::vec3 posVecchia, float offset, bool tutti) {
		...
		TriangoloPt puntoCamera(glm::vec3(posCamera.x, posCamera.y - offset + 2, posCamera.z + 3));

		vector<TriangoloPt> vicino;
		if (tutti == true) {
			vicino = alberoKD.getPuntiEntroRaggio(puntoCamera, 80);
		}
		else {
			vicino = alberoKD.getPuntiEntroRaggio(puntoCamera, 25);
		}
