...
GLuint BgInit_1(int bgshaderProgram, const char* pathTexture) {
...
GLuint BgInit_2(int bgshaderProgram, const char* pathTexture) {
	/*const char* bgfragmentShaderSource_2 = "#version 330 core\n"
		"out vec4 FragColor;\n"
		"in vec2 outTexcoord;\n"
		"uniform sampler2D bgtexture2;\n"
	...
	stbi_set_flip_vertically_on_load(true);
	int width, height, nrChannels;
	unsigned char* data2 = stbi_load(pathTexture, &width, &height, &nrChannels, 0);
	if (data2)
	{
	...
	glUseProgram(bgshaderProgram);
	glUniform1i(glGetUniformLocation(bgshaderProgram, "bgtexture2"), 1);

	return bgtexture;
}

GLuint BgInit_3(int bgshaderProgram, const char* pathTexture) {
	/*const char* bgfragmentShaderSource_2 = "#version 330 core\n"
		"out vec4 FragColor;\n"
		"in vec2 outTexcoord;\n"
		"uniform sampler2D bgtexture2;\n"
...
void Drawcarselect1(GLuint bgtexture, GLuint bgshaderProgram, GLuint bgVAO, Shader* carShader, Model* ourModel, GLFWwindow* window) {
	// -------Render Image-------------------------
	...
	projection2 = glm::perspective(glm::radians(60.0f), (float)1920 / (float)1080, 0.01f, 100.0f);
	glm::mat4 view2 = glm::mat4(1.0f);
	carShader->setMat4("projection", projection2);
	carShader->setMat4("view", view2);

	mat4 model = mat4(1);
	model = translate(model, vec3(0.3f, 0, -1));
	model = scale(model, vec3(0.65f, 0.65f, 0.65f));
...
void Drawcarselect2(GLuint bgtexture, GLuint bgshaderProgram, GLuint bgVAO, Shader* carShader, Model* ourModel, GLFWwindow* window) {
	// -------Render Image-------------------------
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, bgtexture);
	...
	mat4 model = mat4(1);
	model = translate(model, vec3(0.3, 5, -1));
